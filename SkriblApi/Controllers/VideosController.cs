        ...
        public async Task<IHttpActionResult> GetHowItWorksVideoUrl()
        {
            try
            {
                using (SkriblContext ctx = new SkriblContext())
                {
                    ...
                    if (setting != null)
                        return Ok(new CustomResponse<HowItWorksViewModel> { Message = Global.ResponseMessages.Success, StatusCode = (int)HttpStatusCode.OK, Result = new HowItWorksViewModel { Url = setting.HowItWorksUrl, HowItWorksDescription = setting.HowItWorksDescription } });
                    else
            ...
            try
            {
                using (SkriblContext ctx = new SkriblContext())
                {
                    ...
                    if (setting != null)
                    {
                        ...
                        ctx.Settings.Add(new Settings { HowItWorksUrl = model.Url, HowItWorksDescription = model.HowItWorksDescription });
                    ctx.SaveChanges();
