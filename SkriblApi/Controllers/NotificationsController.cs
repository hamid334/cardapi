...
namespace BasketApi.Controllers
{
  //  [BasketApi.Authorize("User", "Guest", "Deliverer","")]
    ...
    public class NotificationsController : ApiController
    {
        ...
        public async Task<IHttpActionResult> GetLatestNotifications()
        {
            try
            {
                using (SkriblContext ctx = new SkriblContext())
                {
                    ...
                    var Notifications = ctx.AdminNotifications.OrderByDescending(x => x.Id).Take(10).ToList();
                    CustomResponse<List<AdminNotifications>> response = new CustomResponse<List<AdminNotifications>> { Message = Global.ResponseMessages.Success, StatusCode = (int)HttpStatusCode.OK, Result = Notifications };
